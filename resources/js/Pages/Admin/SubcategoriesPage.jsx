...
export default function SubcategoriesPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch("/admin/subcategories-data", {
                headers: {
            ...
            setServerError("Failed to load subcategories.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const filteredSubcategories = useMemo(() => {
        return subcategories.filter((item) => {
            const matchesCategory =
                !selectedCategoryFilter || String(item.category_id) === String(selectedCategoryFilter);
    ...
    const handleSubmit = async (e) => {
        ...
        const url = form.id ? `/admin/subcategories/${form.id}` : "/admin/subcategories";
        ...
        try {
            ...
            if (!response.ok) {
                if (data.errors) {
                    ...
                    setServerError(data.message || "Failed to save subcategory.");
                }
                return;
            }
            ...
            setServerError("Something went wrong while saving the subcategory.");
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            if (!response.ok) {
                setServerError(data.message || "Failed to delete subcategory.");
                return;
            }
            ...
            setServerError("Something went wrong while deleting the subcategory.");
        }
    };
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                            <div>
                                ...
                                <p className="text-slate-300 mt-3 max-w-2xl">
                                    Organize your catalog with clean subcategory groups, connect them
                                    to categories, and keep storefront navigation structured.
                                </p>
                            </div>

                            <div className="grid grid-cols-2 gap-4 min-w-[260px]">
                                <div className="rounded-2xl bg-white/10 border border-white/10 p-4 backdrop-blur-sm">
                                    <p className="text-slate-300 text-sm">Total Subcategories</p>
                                    <p className="text-white text-2xl font-bold mt-1">{subcategories.length}</p>
                                </div>
                                <div className="rounded-2xl bg-white/10 border border-white/10 p-4 backdrop-blur-sm">
                                    <p className="text-slate-300 text-sm">Filtered Results</p>
                                    <p className="text-white text-2xl font-bold mt-1">{filteredSubcategories.length}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="rounded-[30px] bg-white border border-[#ece7dc] shadow-[0_20px_50px_rgba(15,23,42,0.08)] overflow-hidden">
                                <div className="bg-gradient-to-r from-[#F68B30] to-[#ffb15c] px-6 py-5">
                                    <h2 className="text-xl font-bold text-white">
                                        {form.id ? "Edit Subcategory" : "Add New Subcategory"}
                                    </h2>
                                    <p className="text-white/90 text-sm mt-1">
                                        Select a parent category and manage subcategory details.
                                    </p>
                                </div>
                            ...
                            <div className="rounded-[30px] bg-white border border-[#ece7dc] shadow-[0_20px_50px_rgba(15,23,42,0.08)] overflow-hidden">
                                <div className="flex flex-col gap-4 px-6 py-5 border-b border-[#f0eadf] bg-[#fffdfa]">
                                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                                        <div>
                                            <h2 className="text-xl font-bold text-[#0f172a]">Subcategories List</h2>
                                            <p className="text-sm text-gray-500 mt-1">
                                                Search, filter, edit, and manage subcategories with ease.
                                            </p>
                                        </div>
                                    ...
                                    <div className="grid md:grid-cols-2 gap-4">
                                        <div>
                                            <input
                                                ...
                                                placeholder="Search by subcategory, slug, or category..."
                                                className="w-full rounded-2xl border border-[#d9d4c7] bg-white px-4 py-3 outline-none focus:border-[#F68B30] focus:ring-4 focus:ring-[#F68B30]/10"
                                            />
                                        </div>
                                ...
                                <div className="p-6">
                                    {loading ? (
                                        ...
                                        <div className="rounded-2xl bg-[#fcfbf8] border border-dashed border-[#d9d4c7] px-5 py-12 text-center">
                                            <p className="text-lg font-semibold text-[#0f172a]">No matching subcategories found</p>
                                                    ...
                                                    <tbody>
                                                        {filteredSubcategories.map((item) => (
                                                            ...
                                                            >
                                                                ...
                                                                <td className="py-4 px-5">
                                                                    <div className="h-14 w-14 rounded-2xl overflow-hidden bg-[#f3efe8] border border-[#ebe3d6] flex items-center justify-center">
